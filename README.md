...
当前内容审计：86 / 86 个合同与 271 / 271 个物品均有可靠显示名；118 / 118 种上交材料均有带来源的获取说明；255 / 271 个条目已有可追踪本地真实游戏图片，其中上交材料为 114 / 118。其余 4 种上交材料没有可靠实图，页面诚实显示统一占位，不用相似图片硬塞。参考图的来源、水印状态和替换优先级只保留在离线映射与交接文档中。
...
- 没有精确实图时优先显示统一占位；经项目所有者明确许可，可临时使用同型号、同系列或真实游戏流程/物品类别画面，但必须标为 `base_model`，公开界面只显示“参考图”，绝不使用 AI 图或把参考图冒充成精确图。

当前 255 / 271 个条目有可追踪的本地真实游戏图片，118 种上交材料中 114 种有图。六排勋章、政府制图局勋章、ASD 安全驱动器、维科洛货币、Yormandi 部位、Vanduul 材料、ATLS、Parallax、Quartz、Fresnel 及多种货运矿物等高频条目均已固定到公开资料页中的精确图片；ATLS 冷铁与 BUL-H4 使用标题或画面能明确确认物品的 RSI Community Hub 实拍。`Carinite (Pure)` 暂使用明确标为“基础型号参考图”的 Carinite 图片；刷新失败时不会被占位图覆盖。4.10 新增或调整后仍缺可靠图的 `Metamaterial Test #152` 与 Strata 臂甲、胸甲、腿甲继续显示占位，避免错误图片误导玩家。
...
H4-PBF Ammo Carrier、Metamaterial Test #146、基础 Monde 四个护甲分件、Saldynium (Ore) 和 Warden Backpack Monde 仍未找到精确物品图，现按项目所有者要求临时使用真实游戏参考图：H4 系列实拍、玩家实际合成界面、CIG 官方 Monde Keystone 整套图、玩家矿石货物图和 Warden 基础型号图。8 条映射均记录原始来源、SHA-256、水印状态与替换优先级；公开界面只显示“参考图”，轮巡发现更准确素材时直接替换。
