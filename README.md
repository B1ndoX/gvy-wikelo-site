...
当前内容审计：86 / 86 个合同与 271 / 271 个物品均有可靠显示名；118 / 118 种上交材料均有带来源的获取说明和本地图片。265 / 271 个条目有图，剩余 6 个无图条目全部是蓝图；32 个实体条目使用带小型“AI重绘”角标的 Image 2 本地重绘，参考来源、成图哈希、水印状态和重绘说明只保留在离线映射与交接文档中。
...
- 精确、清晰且比例合适的真实游戏图仍是第一选择。已有真实素材能确认具体物品，但因清晰度、构图、背景或画幅不适合网站时，允许使用 Image 2 基于原素材重绘成适合对应类别的站内图片；不得改变物品型号、配色、结构、涂装或关键辨识特征。此类图片在数据层标为 `ai_redraw`，页面图片右上角自动显示很小的“AI重绘”角标，不能冒充精确原始截图。
...
当前 265 / 271 个条目有本地图片，118 / 118 种上交材料有图；剩余 6 项全部是蓝图记录，不虚构独立实体封面。精确实图继续优先，32 个实体条目因缺少合适独立图或原图不适合网站展示而标记为 `ai_redraw`；刷新失败时不会被占位或旧参考图覆盖。
...
2026-08-28 起，缺少独立实图或现有参考图不适合直接展示的实体物品使用 Image 2 本地重绘。重绘严格保留可核验参考图与来源，数据层标记为 `ai_redraw`，公开图片右上角仅显示很小的“AI重绘”角标；蓝图条目不虚构独立封面。当前 32 个实体物品映射到 26 张高质量 WebP，非蓝图上交物已无缺图，后续轮巡找到更准确实图时仍优先替换重绘。
