...
当前本地内容审计：87 / 87 个合同与 268 / 268 个物品均有可靠显示名；119 / 119 种上交材料均有带来源的获取说明；259 / 268 个条目已有可追踪本地真实游戏图片，其中上交材料为 119 / 119。8 种暂未找到精确物品图的上交材料使用已核验来源的临时参考图，并统一标为 `base_model`；其来源、水印状态和替换优先级只保留在离线映射与交接文档中。本段所述新增图片尚未部署。
...
当前 259 / 268 个条目有可追踪的本地真实游戏图片，119 种上交材料已全部有图。六排勋章、政府制图局勋章、ASD 安全驱动器、维科洛货币、Yormandi 部位、Vanduul 材料、ATLS、Parallax、Quartz、Fresnel 及多种货运矿物等高频条目均已固定到公开资料页中的精确图片；ATLS 冷铁与 BUL-H4 使用标题或画面能明确确认物品的 RSI Community Hub 实拍。`Carinite (Pure)` 暂使用明确标为“基础型号参考图”的 Carinite 图片；刷新失败时不会被占位图覆盖。
...
正式域名已核验返回 87 笔交易与 `4.9.0 LIVE` 数据；桌面与 390×844 手机视口均无页面级横向溢出，已加载图片无损坏，材料检索与获取方式弹窗正常，640×462 的 Ursa 地面载具图在详情中使用宽幅框，浏览器控制台无 error 或 warning。GitHub Pages 未启用。
