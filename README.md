...
当前公开内容审计：87 / 87 个合同与 268 / 268 个物品均有可靠显示名；119 / 119 种上交材料均有带来源的获取说明；238 / 268 个条目已有可追踪本地实图，其中上交材料为 98 / 119。其余 21 种上交材料保持明确无图状态，禁止用错图或生成图补位。
...
当前 238 / 268 个条目有可追踪的本地实图，119 种上交材料中有 98 种实图。六排勋章、政府制图局勋章、ASD 安全驱动器、维科洛货币、Yormandi 部位、Vanduul 材料、ATLS、Parallax、Quartz、Fresnel 及多种货运矿物等高频条目均已固定到公开资料页中的精确图片；ATLS 冷铁与 BUL-H4 使用标题或画面能明确确认物品的 RSI Community Hub 实拍。`Carinite (Pure)` 暂使用明确标为“基础型号参考图”的 Carinite 图片；刷新失败时不会被占位图覆盖。

本轮新增精确的 Venture Core、Venture Legs、Jaclium (Ore) 和 Vendetta HMG 图片，并修正公共 API 曾把 Venture Arms 图误配给 Venture Legs 的问题。剩余 21 种上交材料没有找到可核验、无水印且型号一致的实图，主要是 RCMBNT 合成样本、基础 Monde/Strata 分件、Saldynium/Sadaryx、Bluemoon Fungus、Metamaterial Test #146 等；内容审计持续列出其稳定 ID，更新时只接受能确认具体型号的公开图片。
